...
def infos_to_notion(infos: dict) -> bool:
    ...
    page: Block = client.get_block((
        "https://www.notion.so/giulianbiolo/"
        "011e1a10299e43acb76dddabd4b578e5?v="
        "849a9212e3354b8c8b158361265bf600"
    ))
    if page.locked is True:
        page.locked = False
    collection_view: CollectionView = client.get_collection_view((
        "https://www.notion.so/giulianbiolo/"
        "011e1a10299e43acb76dddabd4b578e5?v="
        "69a914defca9426299d15c0b9d4dac9f"
    ))
...
def check_infos_integrity(infos: dict) -> bool:
    ...
    if (isinstance(infos['title'], str) is False
        or (isinstance(infos['price'], str) is False
        and isinstance(infos['price'], float) is False
    )):
        return False
    try:
